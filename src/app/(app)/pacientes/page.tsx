  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      if (user) {
        ...
        // 2. Fetch patients for the current user
    ...
    <div className="min-h-screen bg-slate-900 text-slate-100 p-8">
        <div className="container mx-auto">
            <div className="flex justify-between items-center mb-12">
                ...
                <button className="bg-cyan-500 hover:bg-cyan-600 text-white font-bold py-2 px-4 rounded-lg shadow-lg shadow-cyan-500/20 transition-all duration-300 ease-in-out transform hover:scale-105">
                    + Adicionar Paciente
                </button>
            </div>
            ...
            {loading ? (
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {pacientes.map((paciente) => (
                        <div key={paciente.id} className="bg-slate-800/50 border border-slate-700/50 rounded-2xl p-6 shadow-lg hover:border-cyan-500/50 transition-colors">
                            <h3 className="text-xl font-semibold text-teal-400">{paciente.nome_completo}</h3>
                            <p className="text-slate-400 mt-2">{paciente.email}</p>
